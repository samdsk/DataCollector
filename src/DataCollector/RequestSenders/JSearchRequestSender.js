const axios = require("axios");
require("dotenv").config();
const Logger = require("../Loggers/CollectorLogger");
const RapidAPICollectionError = require("../Errors/RapidAPICollectionError");
const LanguageUtils = require("../Utils/LanguageUtils");

class JSearchRequestSender {
    static DATA_PROVIDER = "JSearch";
    static API_URL = "https://jsearch.p.rapidapi.com/search";
    static API_HOST = "jsearch.p.rapidapi.com"

    constructor(apiKey) {
        this.apiKey = apiKey;
    }

    setApiKey(apiKey) {
        this.apiKey = apiKey;
    }

    /**
     * Build the query parameters for the API request.
     *
     * @param {string} jobType - The job type query.
     * @param {string} requestedPage - The pagination token (if any).
     * @param num_pages - Max number of pages
     * @param {Object} options - Optional parameters.
     * @returns {Object} The request query parameters.
     */
    buildParams(jobType, requestedPage = 1, num_pages = 1, {
        location = LanguageUtils.getCountryCode(process.env.API_LANGUAGE),
        datePosted = "month",
    } = {}) {
        return {
            query: jobType,
            page: requestedPage,
            country: location,
            date_posted: datePosted,
            num_pages
        };
    }

    /**
     * Build the complete request options object.
     *
     * @param {Object} params - The query parameters.
     * @returns {Object} The request options.
     */
    buildRequestOptions(params) {
        return {
            method: "GET",
            url: JSearchRequestSender.API_URL,
            params,
            timeout: 45000,
            headers: {
                "X-RapidAPI-Key": this.apiKey,
                "X-RapidAPI-Host": JSearchRequestSender.API_HOST,
            }
        };
    }

    /**
     * Format the API response by injecting additional context.
     *
     * @param {Object} data - The raw response data.
     * @param {string} jobType - The job type query.
     * @param {string} location - The location used in the request.
     * @param {string} language - The language used in the request.
     * @returns {Object} The formatted response data.
     */
    formatResponse(data, jobType, location, language) {
        return {
            ...data,
            location,
            language,
            job_type: jobType,
            data_provider: JSearchRequestSender.DATA_PROVIDER,
        };
    }

    /**
     * Format and enrich an error with additional properties.
     *
     * @param {Error} error - The original error.
     * @param {string} jobType - The job type that triggered the error.
     * @param {string} requestedPage - The pagination token used in the request.
     * @returns {Error} The enriched error.
     */
    formatError(error, jobType, requestedPage) {
        return new RapidAPICollectionError(error.message, {
            status: error?.response?.status,
            jobType: jobType,
            requestedPage: requestedPage,
            originalError: error
        });
    }

    /**
     * Sends a GET request to the RapidAPI endpoint.
     *
     * @param {string} jobType - The job type query.
     * @param {string} [requestedPage=""] - Pagination token if available.
     * @param num_pages - Max number of pages
     * @param {Object} [options={}] - Optional parameters.
     * @returns {Promise<Object>} The response data.
     * @throws {Error} Throws an enriched error on request failure.
     */
    async sendRequest(jobType, requestedPage = 1, num_pages = 1, options = {}) {
        const location = options?.location || LanguageUtils.getCountryCode(process.env.API_LANGUAGE)
        const language = options?.language || process.env.API_LANGUAGE

        const params = this.buildParams(jobType, requestedPage, num_pages, {...options, location, language});
        const requestOptions = this.buildRequestOptions(params);

        try {
            Logger.debug(`${JSearchRequestSender.DATA_PROVIDER}: Sending request with params: ${JSON.stringify(params)}`);
            const response = await axios.request(requestOptions);
            return this.formatResponse(response?.data, jobType, location, language);
        } catch (error) {
            console.log(error)
            Logger.error(`${JSearchRequestSender.DATA_PROVIDER}: Receiving : ${JSON.stringify(error)}`);
            if (error.response) {
                throw this.formatError(error, jobType, requestedPage);
            }
            throw error;
        }
    }
}

module.exports = JSearchRequestSender;

